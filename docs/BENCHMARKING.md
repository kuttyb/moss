...
harness uses `std::hint::black_box` at the backend boundary, performs five warmup rounds,
...
compiler version, release profile, platform identity, UTC timestamp, stable benchmark
ID, summary statistics, methodology counts, and all samples. Comparison matches by
stable ID and reports percentage change. Moss emits a structured
`BASELINE_INCOMPATIBLE` warning if compiler, profile, platform, or IDs do not match.
`--fail-over` is optional and returns nonzero with `BENCHMARK_REGRESSION` when a matched
median exceeds the requested percentage. Automated compiler tests validate structure
and methodology but never depend on a real-time threshold.
