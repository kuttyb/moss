...
Rust made memory safety practical by moving problems such as use-after-free and unsafe aliasing into the compiler. Moss applies the same philosophy to shared-memory concurrency: you describe ownership and communication between domains, and the compiler derives the fine-grained locks and their acquisition order for you. Application code never manages locks directly, and lock-order deadlocks are ruled out by construction.

As the codebase grows, the locking strategy scales with the program instead of becoming another manually maintained part of it.

From Python, Moss takes a preference for small amounts of syntax: indentation-based blocks, type inference, simple functions, direct object construction, structural typing, and readable collection pipelines.

From Rust and other systems languages, Moss takes native compilation, ownership-aware values, static checking, and efficient shared-memory execution.

Where Moss tries to go further is shared-memory concurrency.

In a large shared-memory program, writing a lock is easy. Maintaining hundreds of fine-grained locks without accidentally creating forgotten locking, inconsistent lock ordering, or deadlocks is much harder. The difficulty grows with the code base.

Moss does not ask the programmer to write those locks.

You describe mutable shared state using **domains** and synchronous messages. The compiler examines which pieces of state each handler reads and writes, derives fine-grained synchronization, and generates the locks and lock ordering itself. The synchronization plan uses a compiler-controlled global acquisition order, so application code cannot accidentally introduce lock-order deadlocks.

That means ordinary Moss code does not contain:
...
The compiler owns that machinery.

So a Python programmer can start with code that looks familiar:
...
and still use the same language for shared-memory systems code:
...
There is no lock around `value` in the source. Moss derives the required synchronization from the program.

The goal is not to make programmers write Rust with different punctuation. Moss tries to move difficult systems machinery out of application code and into the compiler. There are no user-visible lifetimes, borrow annotations, lock APIs, actor schedulers, or runtime trait objects.
...
Domains are where Moss moves beyond ordinary Python-like programming.

A domain owns mutable state:
