...
| AB019 | `FUNCTIONAL_SEMANTIC_ERROR` | `Checker::check_functional_callable` | parsed zero-argument invocation names an existing function while the stage requires a callable identity | `FUNCTIONAL_CALLABLE_INVOCATION_UNSUPPORTED`; same rule; `named-pipeline-callable` guidance | `map(double())`, `map(double)` rewrite, and unrelated unknown callable negative case |
